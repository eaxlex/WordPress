...
class Config {
	...
	const Google = true;
	const loader = '(function(d, s, i) { var f = d.getElementsByTagName(s)[0], j = d.createElement(s);j.async = true;
j.src = "https://t.themarketer.com/t/j/" + i; f.parentNode.insertBefore(j, f); })(document, "script", "%s")';

	const defMime = 'xml';

	const discountRules = array(
		0 => 'fixed_cart',
		1 => 'percent',
		2 => 'free_shipping',
	);
...
importScripts("https://t.themarketer.com/firebase.js");';
	...
	public static function tableName() {
		if ( self::$MKTR_TABLE == null ) {
			self::$MKTR_TABLE = self::db()->prefix . 'mktr_session'; }
		return self::$MKTR_TABLE;
	}

	public static function db() {
		if ( self::$MKTR_DB == null ) {
			global $wpdb;
			self::$MKTR_DB = $wpdb; }
		return self::$MKTR_DB;
	}
	...
	public static function POST( $key ) {
		if ( isset( $_POST[ $key ] ) && \wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['mktr_check'] ) ), 'mktr_check' ) ) {
			if ( self::$name === $key && isset( $_POST[ $key ] ) && is_array( $_POST[ $key ] ) ) {
				$list = array();
	...
	public static function getValue( $name = null ) {
		...
		if ( array_key_exists( $name, self::configNames ) ) {
			self::$configValues[ $name ] = get_option( 'mktr_wp_' . self::configNames[ $name ], null );

			if ( self::$configValues[ $name ] === null ) {
				update_option( 'mktr_wp_' . self::configNames[ $name ], self::configDefaults[ $name ] );
				self::$configValues[ $name ] = get_option( 'mktr_wp_' . self::configNames[ $name ], null );
			}
	...
	public static function setValue( $name, $value ) {
		if ( array_key_exists( $name, self::configNames ) ) {
			update_option( 'mktr_wp_' . self::configNames[ $name ], $value );
			self::$configValues[ $name ] = get_option( 'mktr_wp_' . self::configNames[ $name ], null );
		} else {
	...
	public static function cleanSetup() {
		foreach ( self::configNames as $value ) {
			delete_option( $value );
		}
